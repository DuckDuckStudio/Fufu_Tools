...
result_folder_path = os.path.join(os.getcwd(), '查找结果.txt')
        ...
        for file in files:
            ...
            try:
                ...
                if search_text in content:
                    files_found.append(folder_path)

            except Exception as e:
                print(Fore.RED + f"\n[ERROR]无法处理文件：{folder_path}\n因为： {e}")
...
# 将结果写入文件
with open(result_folder_path, 'w', encoding='utf-8') as result_file:
    for folder_path in found_files:
        result_file.write(folder_path + '\n')

print("\n程序运行结束，请查看程序所在目录下生成的文件！")
os.system("start " + result_folder_path)
input("按 ENTER 键继续...")
